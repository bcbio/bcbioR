...
# devtools::install_version(c('Matrix'), c('1.6.4'))
# devtools::install_version(c('SeuratObject'), c('5.0.2'))
# devtools::install_version(c('Seurat'), c('5.1'))
# to-do: write more informative msg to direct the upgrade seurat version
```



```{r load_libraries, cache = FALSE, message = FALSE, warning=FALSE, echo=FALSE,}
pacman::p_load(bcbioR,knitr,
               glue,qs,dplyr,purrr,
               ggplot2,ggprism,grafify,ggpubr,
               gridExtra,scales,
               Seurat)
...
Those two metrics is really dependent on tissue type, RNA quality, and sequencing depth. Since the test data is generated from Visium HD technology, we use bin and corresponding reference thresholds in the plot. Two reference lines 20 and 100 are plotted as the suggested cut-offs. 100 UMI per bin is suggested from 10x website and a 5 fold difference between two metrics are taken here.

```{r}
summary_metaD <- apply(metaD[,-1],2,mean)
metacol_label <- list("nFeature"="Genes","nCount"="UMI")
refs <- list("nFeature"=20,"nCount"=100)
...
cutoffs <- list("nFeature"=20,"nCount"=100,"hb"=20,"mito"=20)
