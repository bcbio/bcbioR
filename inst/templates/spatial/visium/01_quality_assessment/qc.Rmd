...
merged@meta.data %>%
    ...
    geom_violin(aes(fill=orig.ident), alpha = 0.5) +
...
# Clusters ontop slides

```{r cluster_slides fig.height=15}
SpatialDimPlot(merged, label = TRUE, ncol=1)
```

# Save seurat object

```{r save_seurat}
saveRDS(merged, file.path(params$results_dir, "01_qc.RDS"))
```
# Methods
