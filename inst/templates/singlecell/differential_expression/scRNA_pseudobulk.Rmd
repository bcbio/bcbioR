...
for (contrast in names(de_list)){
  ...
  p1=ggplot(top_n_exp, aes_string(x = column, y = 'normalized_counts')) +
...
for (contrast in names(de_list)){
  ...
  seurat_topn_meta <- seurat_topn@meta.data %>% as.data.frame() %>% rownames_to_column('barcode')
  
  topn_counts_df <- LayerData(object = seurat_topn, assay = "RNA", layer = "counts") %>% 
    as.data.frame() %>% rownames_to_column('gene_id') %>% 
    pivot_longer(!gene_id, names_to = 'barcode', values_to = 'counts') %>%
      left_join(seurat_topn_meta)
  
  p2 <- ggplot(topn_counts_df, aes_string(x = column, y = 'counts')) +
      geom_boxplot(outlier.shape = NA, linewidth=0.5, color="grey") + 
      geom_point() +
      facet_wrap(~gene_id, scales = 'free_y') + 
      ggtitle(str_interp('Expression of Top ${n} DEGs - Single Cell')) +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
